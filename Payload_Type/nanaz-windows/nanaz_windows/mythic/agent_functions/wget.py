...
class WgetArguments(FileBrowserArguments):
    ...
    def __init__(self, command_line, **kwargs):
        ...
        self.args = [
            ...
            CommandParameter(
                ...
                type=ParameterType.String,
                default_value="",
                description="Compatibility alias for path when tasking via JSON/API.",
            ),
            CommandParameter(
                name="max_bytes",
    ...
    async def parse_arguments(self):
        ...
        if cl.startswith("{"):
            ...
            destination = data.get("path") or data.get("output")
            if destination:
                self.set_arg("path", destination)
...
class WgetCommand(CommandBase):
    ...
    async def create_go_tasking(self, taskData: PTTaskMessageAllData) -> PTTaskCreateTaskingMessageResponse:
        ...
        if not path and output:
            path = output
            taskData.args.set_arg("path", path)
        taskData.args.remove_arg("output")
