...
def adjust_file_name(
    filename: str,
    shellcode_format: str,
    output_type: str,
    adjust_filename: bool,
    selected_os: str,
) -> str:
    ...
    try:
        await SendMythicRPCPayloadUpdatebuildStep(
            MythicRPCPayloadUpdateBuildStepMessage(
                PayloadUUID=payload_uuid, StepName=step_name, StepSuccess=success
            )
        )
...
class Nanaz(PayloadType):
    ...
    build_steps = [
        BuildStep(
            ...
            step_description="Parsing options and generating config.json",
        ),
        BuildStep(
            step_name="Compiling",
            step_description="Compiling Rust source code via Cargo",
        ),
        BuildStep(
            step_name="Processing Output",
            step_description="Handling final artifact transformation",
        ),
        BuildStep(
            step_name="Finalizing",
            step_description="Adjusting payload details and completing build",
        ),
    ]
    ...
    async def build(self) -> BuildResponse:
        ...
        try:
            ...
            target_os = (
                "Windows" if "windows" in str(target_os_raw).lower() else "Linux"
            )
            ...
            for c2 in self.c2info:
                ...
                if profile_name == "http":
                    ...
                    for key, val in c2.get_parameters_dict().items():
                        if key == "AESPSK":
                            profile_params["aes_psk"] = (
                                val.get("enc_key") if val is not None else ""
                            )
                        else:
                            profile_params[key] = val

                    # 用 profile 名称作为 Key 存入字典
                    c2_profiles.append({profile_name: profile_params})
            ...
            if target_triple:
                ...
                binary_path = (
                    self.agent_code_path / "target" / profile_dir / rust_binary_name
                )
